...
export default function ActiveChat(props) {
  ...
  function updateChatAndNewMessageFromWS(chatAndMessage) {
    ...
    // console.error(!messages.some(x => x.id === message.id))

    if (message) {
      setTimeout(() => {
        setMessages((prevMessages) => [...prevMessages.filter(x => x.id !== message.id), message])
...
async function loadMessages(
  ...
  await jwtRequest
    ...
    .then((res) => {
      ...
      setMessages((prevMessages) => [...newMessages, ...prevMessages.filter(x => !newMessages.some(z => z.id === x.id))])
